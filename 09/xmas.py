...
def any_pair_sums_to(numbers, pair_sum):
    for (index, num) in enumerate(numbers):
        for (index2, num2) in enumerate(numbers):
            if num + num2 == pair_sum and index != index2:
                return True
    return False
...
def find_sub_array_with_sum(array, sum_in_array):
    for i in range(len(array)):
        current_sum = array[i]
        j = i + 1
        while j <= len(array):
            if current_sum == sum_in_array:
                return i, j - 1
            if current_sum > sum_in_array or j == len(array):
                break
